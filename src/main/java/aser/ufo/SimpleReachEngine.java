...
public class SimpleReachEngine {
  ...
  public void addEdge(AbstractNode from, AbstractNode to) {
    ...
    spawn.addTo(to.tid, Bytes.longs.add(from.gid, to.gid));
  }


  // DFS
  private long searchEdge(short t1, int gid1, short t2) {
  ...
  public boolean canReach(AbstractNode n1, AbstractNode n2) {
    ...
    final int gid1 = n1.gid;
    final int gid2 = n2.gid;
    if (t1 == t2) {
      // gid grows within one thread
      return gid1 >= gid2;
    } else { // diff thread

      final long gids = Bytes.longs.add(gid1, gid2);
      synchronized (cache) {
