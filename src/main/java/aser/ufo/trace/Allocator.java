...
//            __df_nodes.add(deallocNode);
  ...
  public void matchInterThrDealloc(SimpleReachEngine reachEngine) {
...
//    System.out.println(">>> matchInterThrDealloc after unmatched " + unmatched.size() + "    matched: " + _matchedAlloc());
  ...
  private boolean tryMatchPairs(AllocaPair p1, AllocaPair p2, DeallocNode deNode, SimpleReachEngine reachEngine) {
    AllocNode a1 = p1.allocNode;
    AllocNode a2 = p2.allocNode;
  ...
  public boolean checkAcc(MemAccNode accNode, SimpleReachEngine reachEngine) {
    ...
    for (Short2ObjectMap.Entry<TLAllocQ> e : tid2AllocQ.short2ObjectEntrySet()) {
      ...
      for (ArrayList<AllocaPair> ls : lss)
        for (AllocaPair p : ls) {
          ...
          if (dn == null || dn.tid == accTid)
            continue;
          AllocNode an = p.allocNode;
          if (an.addr <= accAddr
              && accAddr < an.addr + an.length
