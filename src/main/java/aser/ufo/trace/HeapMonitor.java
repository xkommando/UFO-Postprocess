...
public class HeapMonitor {
  ...
  public void allocated(AllocNode allocNode) {
    ...
    ap.allocSeq.put(allocNode.idx, allocNode);
    allocCount++;
  }
  ...
  public void pairDealloc() {
    ...
    for (Long2ObjectMap.Entry<AllocPoint> e1 : addrOpMap.long2ObjectEntrySet()) {
      ...
      for (DeallocNode dn : deLs) {
        Long2ObjectSortedMap<AllocNode> aLs = ap.allocSeq.headMap(dn.idx);
        if (aLs.isEmpty()) {
          LOG.debug(" Empty alloc for dealloc idx:{} addr: {}, tid {}", dn.idx, dn.addr, dn.tid);
        } else {
          AllocNode an = aLs.get(aLs.lastLongKey());
          if (an.addr == dn.addr) {
            dn.length = an.length;
            gAllocSeq.put(an.idx, new AllocPair(an, dn));
            pairCount++;
          } else {
...
//    System.out.println(pn);
  ...
  public AllocPair searchAllocPair(MemAccNode accNode) {
    ...
    // second attempt
    Long2ObjectSortedMap<AllocPair> earlyMap = gAllocSeq.tailMap(accNode.idx);
